...
inline uint8_t Comms::Comms::crc(const uint8_t *bytes, const uint16_t length) {
  uint8_t sum = 0;
...
// TODO: WTF why? It works on ARM Platform but not on AVR! Same implementation on both devices
  ...
  if (rx_packet->header.sync == SYNC && rx_packet->header.crc == calculated_checksum) {
    AccessRequest access_response = {
            .state = static_cast<AccessRequestType>(rx_packet->header.access_type & 0b00000000000011),
            .sensor = static_cast<AccessRequestType>((rx_packet->header.access_type & 0b0000000000001100) >> 2),
            .data = static_cast<AccessRequestType>((rx_packet->header.access_type & 0b0000000000110000) >> 4),
            .parameter = static_cast<AccessRequestType>((rx_packet->header.access_type & 0b0000000011000000) >> 6),
            .request = static_cast<AccessRequestType>((rx_packet->header.access_type & 0b0000001100000000) >> 8),
    };

    uint16_t rx_index = 0;
    // Coppy the valid data witch was generated by the access request from the last cycle into the local ram
    if (access_response.state == AccessRequestType::GET) {
      memcpy(&data->state, &rx_packet->data.buffer[rx_index], sizeof(State::State));
      rx_index += sizeof(State::State);
    }
    if (access_response.sensor == AccessRequestType::GET) {
      memcpy(&data->sensor, &rx_packet->data.buffer[rx_index], sizeof(Sensor::Sensor));
      rx_index += sizeof(Sensor::Sensor);
    }
    if (access_response.data == AccessRequestType::GET) {
      memcpy(&data->data, &rx_packet->data.buffer[rx_index], sizeof(Data::Data));
      rx_index += sizeof(Data::Data);
    }
    if (access_response.parameter == AccessRequestType::GET) {
      memcpy(&data->parameter, &rx_packet->data.buffer[rx_index], sizeof(Parameter::Parameter));
      rx_index += sizeof(Parameter::Parameter);
    }
    if (access_response.request == AccessRequestType::GET) {
...
uint8_t Comms::CommsSlave::exchange() {
  ...
  switch (access_request.state) {
    case AccessRequestType::GET:
      memcpy(&tx_packet->data.buffer[tx_index], &data->state, sizeof(State::State));
      tx_index += sizeof(State::State);
      break;
    case AccessRequestType::SET:
      memcpy(&data->state, &rx_packet->data.buffer[rx_index], sizeof(State::State));
      rx_index += sizeof(State::State);
    case AccessRequestType::IGNORE:
      break;
  }
